export const idlFactory = ({ IDL }) => {
  ...
  const RecordKey = IDL.Record({ 'ledger' : LedgerId, 'account' : Account });
  ...
  const SPI103IcrcWalletDemo = IDL.Service({
    'raw_balance' : IDL.Func([Account, IDL.Principal], [IDL.Nat], ['query']),
    'raw_entries' : IDL.Func(
        [],
        [IDL.Vec(IDL.Tuple(RecordKey, IDL.Nat))],
        ['query'],
      ),
