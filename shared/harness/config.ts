...
export const CONFIG_PATH = resolveConfigFile();
export const E2E_DIR = dirname(CONFIG_PATH);
...
export function testConfig(): Required<TestConfig> {
  ...
  return {
    ...
    exceptionsFile: test.exceptionsFile ?? "exceptions.ts",
  };
}
...
export function actorKeyFromArg(arg?: string): string {
  ...
  if (defaultActor === undefined || defaultActor.length === 0) {
    throw new Error(`No actor key given and ${CONFIG_PATH} has no defaultActor.`);
  }
  return defaultActor;
}
...
function resolveConfigFile(): string {
  const configured = process.env.E2E_CONFIG ?? process.env.SR9_E2E_CONFIG;
  ...
  const defaultDexConfig = resolve(REPO_ROOT, "standard_icrc/dex/test/config.json");
  if (existsSync(defaultDexConfig)) {
    return defaultDexConfig;
  }
