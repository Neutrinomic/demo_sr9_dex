# DEX Project E2E Tests

This folder contains the DEX project's runtime specs, generated actor fixture,
and project config. The reusable PocketIC harness lives at the repository root
under `shared/`.

The shared harness borrows the practical parts of the DVF test layout:
deterministic identities, real ICRC ledger deployment, explicit caller
switching, and small helpers for common Candid result variants. The generic
runtime helpers also expose DVF-style time and block control:
...
bun run shared/harness/runner/runE2E.ts large_population
```
...
`bun run typecheck` uses the root `tsconfig.json`, `typescript`, and Bun types,
which is also what VS Code should pick up for files under this repository.
`bun run test` intentionally does not compile. It runs the quiet e2e runner, which
...
```text
standard_icrc/dex/test/fixtures/actors/dex/DexActorDemo.wasm
standard_icrc/dex/test/fixtures/actors/dex/DexActorDemo.did
standard_icrc/dex/test/fixtures/actors/dex/DexActorDemo.idl.js
```
...
By default, actor fixtures go to this project's
`fixtures/actors/<actor-key>/`. Actor-specific deploy helpers should live
beside that actor's generated WASM and DID files.
Scenario setup belongs in specs or shared app-specific test helpers such as
`spec/support/`; the files under `shared/harness/` should not need project
edits.
...
1. Keep project runtime specs under that project's `test/spec/`.
2. Keep reusable runtime helpers in `shared/common/`.
3. Keep reusable runner/build code in `shared/harness/`.
4. Put app-specific actor deployment beside that actor under the project's
   `test/fixtures/actors/<actor-key>/`.
5. Keep project-specific config and exceptions in that project's `test/`.
6. Keep actual `*.pic.ts` tests short and scenario-focused.
7. Use real ledger calls for deposits, withdrawals, approvals, and balance
