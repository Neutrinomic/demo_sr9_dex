...
The repository also includes a shared PocketIC test harness under `shared/`.
The DEX project owns its runtime config and specs under
`standard_icrc/dex/test/`. These tests are not a replacement for SR9 proofs;
they are a black-box adversarial runtime check that assumes bugs may still
exist.
